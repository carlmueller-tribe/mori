...
class MemoryModule:
    ...
    async def write(self, records: list[MemoryRecord]) -> WriteReceipt:
        ...
        return WriteReceipt(record_ids=ids, layer=layer, timestamp=datetime.now(UTC))
    ...
    async def forget(self, policy: ForgetPolicy | None = None) -> ForgetReport:
        ...
        for layer in MemoryLayer:
            ...
            for r in records:
                if (
                    policy.expire_ttl
                    and r.ttl_seconds is not None
                    and (now - r.created_at).total_seconds() > r.ttl_seconds
                ):  # noqa: E501
                    to_delete.append(r.record_id)
                    expired += 1
                    continue
                if (
                    policy.prune_below_confidence is not None
                    and r.confidence < policy.prune_below_confidence
                ):  # noqa: E501
                    to_delete.append(r.record_id)
                    pruned += 1
                    continue
    ...
    async def stats(self) -> MemoryStats:
        ...
        return MemoryStats(
            total_records=total,
            records_per_layer=per_layer,
            estimated_tokens_per_layer=tokens,

...
async def test_promote(module):
    ...
    new_ids = await module.promote(
        MemoryLayer.WORKING, MemoryLayer.SEMANTIC, [MemoryRecordId("rec_1")]
    )
...
async def test_forget_expires_ttl(module):
    expired = _make_record(
        "rec_old", ttl_seconds=1, created_at=datetime.now(UTC) - timedelta(hours=1)
    )
    await module.write([expired])
    report = await module.forget(
        ForgetPolicy(expire_ttl=True, prune_below_confidence=None, deduplicate=False)
    )
...
async def test_forget_prunes_low_confidence(module):
    await module.write(
        [_make_record("rec_low", confidence=0.1), _make_record("rec_high", confidence=0.9)]
    )
    report = await module.forget(
        ForgetPolicy(expire_ttl=False, prune_below_confidence=0.5, deduplicate=False)
    )
...
async def test_stats(module):
    await module.write(
        [
            _make_record("r1", layer=MemoryLayer.WORKING),
...
async def test_summarize_layer_fallback(module):
    await module.write(
        [
            _make_record("r1", layer=MemoryLayer.WORKING, content="Auth uses JWT"),

...
async def test_turn_start_context_injection_live():
    """A turn.start hook mutates the input payload; the model sees the mutation.

    Per Spec 10 A.3.2 (capability matrix), turn.start is a transform event:
    returning a mutated payload from the hook must propagate to the model.
    """
    agent = (
        Mori.builder()
        .model("anthropic", model="claude-haiku-4-5-20251001")
    ...
    assert "SECRET-CANARY-9381" in (result.final_output or ""), (
        f"turn.start mutation did not reach the model. Final output: {result.final_output!r}"
    )

...
async def test_retrieve_respects_max_tokens():
    backend = await _populated_backend()
    result = await retrieve(
        query="test", task_context="", backend=backend, embedder=FakeEmbedder(), max_tokens=10
    )
...
async def test_retrieve_empty_backend():
    result = await retrieve(
        query="test", task_context="", backend=InMemoryBackend(), embedder=FakeEmbedder()
    )

...
from mori.types import (MemoryConfig, MemoryLayer, Message, ModelResponse, RunStatus, ToolCall, TokenUsage)
...
def _text_response(text):
    return ModelResponse(message=Message(role="assistant", content=text),
        usage=TokenUsage(input_tokens=50, output_tokens=20), stop_reason="end_turn")

def _tool_response(tid, name, args):
    return ModelResponse(message=Message(role="assistant", content="",
        tool_calls=[ToolCall(id=tid, name=name, arguments=args)]),
        usage=TokenUsage(input_tokens=50, output_tokens=20), stop_reason="tool_use")
...
async def test_loop_writes_working_memory(mock_model, memory_module):
    mock_model.invoke = AsyncMock(side_effect=[_tool_response("c1", "add", {"a":1,"b":2}), _text_response("3")])
...
async def test_loop_emits_memory_events(mock_model, memory_module):
    ...
    loop = AgentLoop(model=mock_model, tools=ToolRegistry(), memory=memory_module, observability=obs)
    await loop.run("test")
    await obs.flush()

...
async def test_allowed_tool_completes(mock_model):
    ...
    loop = AgentLoop(
        model=mock_model, tools=registry, permission=_allow_engine(), identity=identity
    )
...
async def test_denied_tool_returns_error_result_and_continues(mock_model):
    ...
    loop = AgentLoop(
        model=mock_model, tools=registry, permission=_deny_engine("deploy_prod"), identity=identity
    )
...
async def test_escalate_pauses_run(mock_model):
    mock_model.invoke = AsyncMock(
        return_value=_tool_resp("c1", "deploy_prod", {"version": "1.2.3"})
    )
    registry = ToolRegistry()
    registry.register("deploy_prod", lambda version: "deployed", description="Deploy")
    identity = Identity(id="agent:bot", name="bot", type=IdentityType.AGENT)
    ...
    loop = AgentLoop(
        model=mock_model,
        tools=registry,
        permission=_escalate_engine("deploy_prod"),
        identity=identity,

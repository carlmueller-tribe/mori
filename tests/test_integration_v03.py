...
from mori.types import MemoryLayer, Message, ModelResponse, RunStatus, ToolCall, TokenUsage
...
def _text_response(text):
    return ModelResponse(message=Message(role="assistant", content=text),
        usage=TokenUsage(input_tokens=80, output_tokens=30), stop_reason="end_turn")
...
async def test_v03_exit_test():
    ...
    with patch("mori.agent.AnthropicAdapter") as M:
        mock = AsyncMock()
        mock.invoke = AsyncMock(side_effect=[
        ...
        M.return_value = mock
        agent = Mori.builder().model("anthropic", api_key="test").tool(lookup, description="Look up a topic").memory_backend("inmemory").config(max_steps=10).build()
